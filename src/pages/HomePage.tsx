...
function useGridLimit() {
  const [limit, setLimit] = useState(() => {
    ...
    if (w >= 1024) return 8   // 4 cols × 2 rows
    if (w >= 768)  return 6   // 3 cols × 2 rows
...
export default function HomePage() {
  ...
  return (
    <div className="animate-fade-in" style={{ paddingBottom: 16 }}>
      {/* AppBar */}
      <header
        className="sticky top-0 z-10 flex items-center px-4 lg:hidden"
      ...
      <div className="px-4 pt-5 pb-5">
        <blockquote
          className="text-[28px] font-bold leading-[1.45]"
          style={{ color: 'var(--ink-0)', fontFamily: 'var(--font-serif)' }}
        >
          너희는 가만히 있어<br />내가 하나님 됨을 알지어다.
        </blockquote>
        <p className="text-sm mt-2" style={{ color: 'var(--ink-2)' }}>시편 46:10</p>
      </div>
